...
export default function scheduleUiPatch(): Plugin {
  return {
    ...
    transform(code, id) {
      ...
      out = out.replace(/\{groupedSchedules\.length===0\?.*?\}:tab==='orders'/s, `${scheduleBody}:tab==='orders'`)
