  ...
  return {
    ...
    transform(code, id) {
      ...
      out = out.replace(
        ...
        `const[routeFilter,setRouteFilter]=useState<RouteFilter>('interhub');const[categoryFilter,setCategoryFilter]=useState<'Normal'|'Campaign'>('Normal')`,
      )
      ...
      out = out.replace(
        /<section className="schedule-filters">.*?<\/div>}\/>:tab==='orders'/s,
        `${replacement}</>:tab==='orders'`,
      )
